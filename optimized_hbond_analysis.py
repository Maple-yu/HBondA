...
def get_user_input():
    ...
    return topology_file, trajectory_file, distance_cutoff, angle_cutoff
...
def auto_identify_hbond_atoms(u):
    ...
    # 氢原子识别
    try:
        h_atoms = u.select_atoms("name H* or type 11 13")
    except:
        # 如果名称选择失败，只使用类型选择
        try:
            h_atoms = u.select_atoms("type 11 13")
        except:
            # 如果预设类型也失败，尝试基于质量识别氢原子（质量约1.0）
            h_atoms = u.select_atoms("mass < 2.0")
    
    # 氧/氮/氟等可能的供体/受体原子
    try:
        o_atoms = u.select_atoms("name O* N* F* or type 1 8 9 10 12")
    except:
        try:
            o_atoms = u.select_atoms("type 1 8 9 10 12")
        except:
            # 基于质量识别重原子
            o_atoms = u.select_atoms("mass > 10.0")
...
def select_atom_types(u):
    ...
    if choice == "1":
        ...
        try:
            h_atoms = u.select_atoms(h_selection) if h_selection else u.select_atoms("type 11 13")
            o_atoms = u.select_atoms(o_selection) if o_selection else u.select_atoms("type 1 8 9 10 12")
            print(f"\n手动选择结果:")
            print(f"  氢原子: {len(h_atoms):6d} 个")
            print(f"  供体/受体原子: {len(o_atoms):6d} 个")
...
def analyze_frame_hbonds_optimized(frame_data, h_atoms_ids, o_atoms_ids, o_positions, 
...
def analyze_trajectory_hbonds_parallel(u, h_atoms, o_atoms, distance_cutoff=3.5, angle_cutoff=120.0):
    ...
    # 使用进程池并行处理帧
    analyze_frame_partial = partial(analyze_frame_hbonds_optimized, 
...
def calculate_time_correlation_function_vectorized(all_hbonds, max_tau=None):
    ...
    if max_tau is None:
        max_tau = min(50, len(all_hbonds) // 4)
...
def main():
    ...
    try:
        # 获取用户输入
        topology_file, trajectory_file, distance_cutoff, angle_cutoff = get_user_input()
        ...
        if calc_tcf in ['y', 'yes']:
            max_tau = input("最大时间延迟 (默认为帧数的1/4): ").strip()
        ...
        # 保存结果
        print("\n保存分析结果...")
        save_results(frame_times, hbond_counts, lifetime_results, tcf_results, 'optimized_hbond')
