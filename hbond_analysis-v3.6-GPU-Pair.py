...
def read_atom_type_labels(data_file):
    ...
    从Masses部分的注释中提取标签信息，如果注释为空则使用"type-{ID}"作为默认标签

    参数:
        data_file (str): LAMMPS data文件路径

    返回:
        dict: 原子类型ID到标签的映射字典（键为整数类型的ID，值为字符串标签）
    """
    type_labels = {}
    ...
    with open(data_file, 'r') as f:
        for line in f:
            ...
            if in_masses_section:
                if not stripped or stripped.split()[0] in {"Atoms", "Bonds", "Angles"}:
                    in_masses_section = False
                    continue
                ...
                try:
                    ...
                    # 处理标签：优先使用注释，否则生成默认标签
                    if comment:
                        type_labels[atom_type] = comment
                    else:
                        type_labels[atom_type] = f"type-{atom_type}"
